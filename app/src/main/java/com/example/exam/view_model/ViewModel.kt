...
class EntityViewModel(
    ...
    private val _inProgressEvents = MutableLiveData<List<Car>>()
    val getCarsTypesViewModel: LiveData<List<Car>> get() = _inProgressEvents
    private val _getCarsTypesViewModel = MutableLiveData<List<Car>>()
    val analitics: LiveData<List<Car>> get() = _getCarsTypesViewModel
    var entities: LiveData<List<Car>> = MutableLiveData(emptyList());
    ...
    fun requestCar(carType: String) {
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val response = apiService.requestCar(carType).execute()

                if (response.isSuccessful) {
                    // Handle the successful reservation, update the UI or perform any other actions.
                    Log.d(TAG, "Spot reserved successfully")
                    getCarsTypes()

                } else {
                    // Handle the unsuccessful reservation, show an error message or perform any other actions.
                    Log.d(TAG, "Spot reservation unsuccessful: ${response.code()}")
                    updateToastMessage("Reservation error")
                }
            } catch (e: IOException) {
                // Handle IOException, show an error message or perform any other actions.
                Log.e(TAG, "IOException during spot reservation: ${e.message}", e)
                updateToastMessage("Reservation error")
            } catch (e: Exception) {
                // Handle other exceptions, show an error message or perform any other actions.
                Log.e(TAG, "Error during spot reservation: ${e.message}", e)
                updateToastMessage("Reservation error")
            }
        }
    }
    ...
    fun createEntity(
        ...
        val entity = Car(-1, name, team, details, status, participants, type)
        viewModelScope.launch(Dispatchers.IO) {
            try {
    ...
    suspend fun getEntity(id: Int): Car? {
        try {
    ...
    suspend fun getCarsTypes() {
        try {
            Log.d("EntityViewModel", "Fetching in-progress events...")

            withContext(Dispatchers.IO) {
                val response = apiService.getCarsTypes().execute()

                if (response.isSuccessful) {
                    _inProgressEvents.postValue(response.body())
                    Log.d("EntityViewModel", "In-progress events fetched successfully: ${response.body()}")
                } else {
                    // Handle error
                    Log.e("EntityViewModel", "Error fetching in-progress events. Response code: ${response.code()}")
                }
            }
        } catch (e: Exception) {
            // Handle exception
            Log.e("EntityViewModel", "Exception during in-progress events fetching: ${e.message}", e)
        }
    }

    suspend fun getAnalitics() {
        try {

            withContext(Dispatchers.IO) {
                val response = apiService.getAnalitics().execute()

                if (response.isSuccessful) {
                    _getCarsTypesViewModel.postValue(response.body())

                    Log.d("EntityViewModel", "Cars fetched successfully: ${response.body()}")
                } else {
                    // Handle error
                    Log.e("EntityViewModel", "Error fetching cars. Response code: ${response.code()}")
                }
            }
        } catch (e: Exception) {
            // Handle exception
